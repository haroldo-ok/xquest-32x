...
! Standard Mars Header at 0x3C0
        ...
        .ascii  "XQUEST 32X      "              /* module name (16 chars) */
...
main_err:
! debug: paint CRAM entry 0 magenta so a crash is visible on screen
        mov.l   err_cram,r1
        mov.l   err_color,r0
        mov.w   r0,@r1
0:
...
slav_pwm_irq:
        ...
        ! handle PWM IRQ: call C handler (caller-saved regs preserved)
        mov.l   r2,@-r15
        mov.l   r3,@-r15
        mov.l   r4,@-r15
        mov.l   r5,@-r15
